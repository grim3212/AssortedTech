...
public class GravityBlockEntityRenderer implements BlockEntityRenderer<GravityBlockEntity> {
	...
	@Override
	public void render(GravityBlockEntity entity, float partialTicks, PoseStack stack, MultiBufferSource bufferSource, int destroyStage, int alpha) {
		if (entity.shouldShowRange()) {
			...
			AABB aabb = entity.getBlockState().getCollisionShape(entity.getLevel(), entity.getBlockPos()).bounds().inflate(maxLength).deflate(1D);
